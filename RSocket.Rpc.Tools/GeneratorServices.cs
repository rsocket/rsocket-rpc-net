...
namespace RSocket.Rpc.Tools
{
    ...
    internal abstract class GeneratorServices
    {
        ...
        // Guess whether item's metadata suggests RSocket RPC stub generation.
        // When "RSocketRpcServices" is not defined, assume RSocket RPC is not used.
        // When defined, C# uses "none" to skip RSocket RPC, C++ uses "false", so
        ...
        // It is safe to assume that RSocket RPC is requested for any other value.
        protected bool RSocketRpcOutputPossible(ITaskItem proto)
        {
            string gsm = proto.GetMetadata(Metadata.RSocketRpcServices);
    ...
    internal class CSharpGeneratorServices : GeneratorServices
    {
        ...
        {
            bool doRSocketRpc = RSocketRpcOutputPossible(protoItem);
            string filename = LowerUnderscoreToUpperCamel(
                Path.GetFileNameWithoutExtension(protoItem.ItemSpec));

            var outputs = new string[doRSocketRpc ? 2 : 1];
            string outdir = protoItem.GetMetadata(Metadata.OutputDir);
            string fileStem = Path.Combine(outdir, filename);
            outputs[0] = fileStem + ".cs";
            if (doRSocketRpc)
            {
                // Override outdir if kRSocketRpcOutputDir present, default to proto output.
                outdir = protoItem.GetMetadata(Metadata.RSocketRpcOutputDir);
                if (outdir != "")
                {
                    fileStem = Path.Combine(outdir, filename);
                }
                outputs[1] = fileStem + "RSocketRpc.cs";
            }
            return outputs;
        }
    ...
    internal class CppGeneratorServices : GeneratorServices
    {
        ...
        public override string[] GetPossibleOutputs(ITaskItem protoItem)
        {
            ...
            var outputs = new string[doRSocketRpc ? 4 : 2];
            string outdir = protoItem.GetMetadata(Metadata.OutputDir);
            ...
            if (doRSocketRpc)
            {
                ...
                outputs[2] = fileStem + "_rsocket_rpc.pb.cc";
                outputs[3] = fileStem + "_rsocket_rpc.pb.h";
            }
            return outputs;
        }
